export const formatDate = (date: Date) =& gt;
date.toISOString().split("T")[0];

export const getWeekDays = (date: Date, weekStartDay: number = 1) =& gt; {
  ...
  const adjustedDiff = diff & lt; 0 ? diff + 7 : diff;
  ...
  return Array.from({ length: 5 }).map((_, i) =& gt; {
