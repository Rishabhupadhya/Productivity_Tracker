...
import { awardXP } from "../momentum/momentum.service";
...
export const completeHabit = async (habitId: string, userId: string, date?: Date, notes?: string) => {
  ...
  // Award XP
  await awardXP(userId, 'HABIT', {
    streakDays: habit.currentStreak,
    isTeamHabit: !!habit.teamId
  });
...
export const getHabitStats = async (habitId: string, userId: string) => {
  ...
  return {
    ...
    completionRate: habit.successRate,
    successRate: habit.successRate,
    last30DaysCount: last30Days.length,
    last7DaysCount: last7Days.length,
    completionHistory: habit.completions.slice(-90).map(c => ({
